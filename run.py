...
def predict(
    ...
    with tf.Session() as session:
        ...
        while True:
            ...
            x2_sen = vectorizer.vectorize(x2)
            feed_dict = {model.x1: x1_sen, model.x2: x2_sen, model.is_training: False,
                         model.labels: np.array([[int(x3)]])}
            prediction = session.run([model.temp_sim], feed_dict=feed_dict)
            print("Prediction:")
            print(prediction)
            print("Loss:")
            print(session.run([model.loss], feed_dict=feed_dict))
            # print(session.run([tf.convert_to_tensor(x1_sen)], feed_dict=feed_dict))
            # print(session.run([tf.shape(model.embedded_x1)], feed_dict=feed_dict))
            # print(session.run([tf.shape(tf.reshape(model.embedded_x1, [-1, 64]))], feed_dict=feed_dict))
            gradients_1 = tf.gradients(model.loss, model.embedded_x1)
            gradients_2 = tf.gradients(model.loss, model.embedded_x2)
            # print(session.run([tf.reduce_mean(tf.math.multiply(tf.reshape(gradients_1, [-1, 64]), tf.reshape(model.embedded_x1, [-1, 64])), 1)], feed_dict=feed_dict))
            print(session.run([tf.argsort(tf.reduce_mean(
                tf.math.multiply(tf.reshape(gradients_1, [-1, 64]), tf.reshape(model.embedded_x1, [-1, 64])), 1),
                                          axis=-1, direction='ASCENDING', stable=False, name=None)],
                              feed_dict=feed_dict))
            # print(session.run([tf.reduce_mean(tf.math.multiply(tf.reshape(gradients_2, [-1, 64]), tf.reshape(model.embedded_x1, [-1, 64])), 1)], feed_dict=feed_dict))
            print(session.run([tf.argsort(tf.reduce_mean(
                tf.math.multiply(tf.reshape(gradients_2, [-1, 64]), tf.reshape(model.embedded_x2, [-1, 64])), 1),
                                          axis=-1, direction='ASCENDING', stable=False, name=None)],
                              feed_dict=feed_dict))
            break
